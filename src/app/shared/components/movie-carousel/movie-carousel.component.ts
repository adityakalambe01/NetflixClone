...
@Component({
  ...
  standalone: true,
  imports: [MovieCarouselComponent, NgFor, DescriptionPipe, ImagePipe, NgIf],
...
export class MovieCarouselComponent implements OnInit, AfterViewInit {
  ...
  selectedContenet: string | null = null;
  ngOnInit(): void {}
  ngAfterViewInit(): void {
    this.initilizeSwaper();
  }

  private initilizeSwaper() {
  ...
  setHoverMovie(movie: IVideoContent) {
    this.selectedContenet = movie.title ?? movie.name;
  }

  clearHoverMovie() {
    this.selectedContenet = null;
  }
}
